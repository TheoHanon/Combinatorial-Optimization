...
#     @constraint(model, [k = 1:M, (i, j) in valid_pairs], beta[i] - beta[j] + (n-1)*z[i,j,k] <= n-2)
...
#     @constraint(model, [k in 1:M], sum(q[j] * z[i,j,k] for j in J, i in N) <= Q)
...
function solve_OptVax1(n::Int64, m::Int64, D::Array{Float64, 2}, A::Array{Int64, 2}, Q::Int64, C::Vector{Int64}, q::Vector{Int64}, f::Vector{Int64}, p::Int64, B::Float64, R::Vector{Float64}, J_prime::Vector{Int64}, M::Int64)
    ...
    @constraint(model, sum(u[:,k] for k in 1:M) .+ v .<= 1.0)

    # Routing constraints
    @constraint(model, [j in J, k in 1:M], sum(z[i, j, k] for i in N) == u[j,k])

    @constraint(model, [j in J], sum(u[j,k] for k in 1:M) <= 1.0)
