...
function solve_OptVax1(n::Int64, m::Int64, A::Array{Int64, 2}, Q::Int64, C::Vector{Int64}, q::Vector{Int64}, f::Vector{Int64}, p::Int64, B::Float64, R::Vector{Float64}, J_prime::Vector{Int64}, M::Int64) 
    ...
    for l in I
        for k in 1:M
            @constraint(model, sum(z[l, i, k] for i in N) <= y[l])
            @constraint(model, sum(z[l, i, k] for i in N) <= delta[k])
            @constraint(model, sum(z[l, i, k] for i in N) >= y[l] + delta[k] - 1)
        end
    end
    ...
    @constraint(model, (sum(delta[k] for k in 1:M) * p) + sum(y[i] * f[i] for i in I) <= B)
...
function solve_OptVax2(n::Int64, m::Int64, A::Array{Int64, 2}, Q::Int64, C::Vector{Int64}, q::Vector{Int64}, f::Vector{Int64}, p::Int64, B::Float64, R::Vector{Float64}, J_prime::Vector{Int64}, M::Int64) 
